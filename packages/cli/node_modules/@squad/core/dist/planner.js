...
export function buildConsultationPrompt(cfg, message, overview) {
    const roles = Object.keys(cfg.agents).filter((role) => role !== 'planner' && role !== 'default');
...
export function buildSmartChatPrompt(cfg, message, overview) {
    const roles = Object.keys(cfg.agents).filter((role) => role !== 'planner' && role !== 'default');
    const isDirect = cfg.executionMode !== 'worktree';
...
export function tryParsePlanOutput(output, goal) {
    try {
