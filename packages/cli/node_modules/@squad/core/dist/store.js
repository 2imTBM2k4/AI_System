...
export class SquadStore {
    ...
    createPlannedRun(id, repoPath, plan, createdAt = new Date().toISOString()) {
        this.database
            .prepare(`INSERT INTO runs (id, repo_path, goal, plan_json, pid, status, created_at, ended_at)
         VALUES (?, ?, ?, ?, NULL, 'planned', ?, NULL)`)
            .run(id, repoPath, plan.goal, JSON.stringify(plan), createdAt);
    }
    startPlannedRun(id, plan, logPaths, event, pid = process.pid, startedAt = new Date().toISOString()) {
        this.transaction(() => {
    ...
    markTaskRunning(runId, taskId, startedAt) {
        this.database
    ...
    setTaskPid(runId, taskId, pid) {
        this.database
            .prepare('UPDATE tasks SET pid = ? WHERE id = ? AND run_id = ?')
            .run(pid, taskId, runId);
    }
    recordTaskStarted(runId, taskId, startedAt, event) {
        this.transaction(() => {
            this.markTaskRunning(runId, taskId, startedAt);
            this.appendEvent(event, startedAt);
        });
    }
    saveTaskResult(runId, result) {
        this.database
            .prepare(`UPDATE tasks
         SET status = ?, started_at = ?, ended_at = ?, error = ?, pid = NULL
         WHERE id = ? AND run_id = ?`)
            .run(result.status, result.startedAt, result.endedAt ?? null, result.error ?? null, result.id, runId);
    }
    recordTaskResult(runId, result, event, createdAt = result.endedAt ?? new Date().toISOString()) {
        this.transaction(() => {
            this.saveTaskResult(runId, result);
            this.appendEvent(event, createdAt);
        });
    }
    appendEvent(event, createdAt = new Date().toISOString()) {
        this.database
            .prepare(`INSERT INTO events (run_id, task_id, type, payload, created_at)
         VALUES (?, ?, ?, ?, ?)`)
            .run(event.runId, taskIdForEvent(event), event.type, JSON.stringify(event), createdAt);
    }
    completeRun(runId, status, results, endedAt = new Date().toISOString()) {
        const event = {
            type: 'run:done',
            runId,
            results,
    ...
    getRunPlan(id) {
        ...
        return JSON.parse(toStringValue(row.plan_json));
    }
    listTasks(runId) {
    ...
    interruptRun(runId, endedAt = new Date().toISOString()) {
        ...
        this.transaction(() => {
            ...
            this.database
                .prepare("UPDATE tasks SET status = 'interrupted', ended_at = ?, error = ?, pid = NULL WHERE run_id = ? AND status IN ('running', 'pending')")
    ...
    toRunRecord(row) {
        return {
            ...
            plan: row.plan_json === null ? null : JSON.parse(toStringValue(row.plan_json)),
