...
export class PlanError extends Error {
  readonly code = 'PLAN_INVALID' as const;
...
export function buildPlannerPrompt(cfg: SquadConfig, goal: string, overview: RepoOverview): string {
...
export function parsePlanOutput(output: string, goal: string): Plan {
  let parsed: unknown;
  try {
...
export function validatePlan(value: unknown, goal: string): Plan {
...
export function detectCycle(tasks: readonly Task[]): void {
...
export function topoSort(tasks: readonly Task[]): Task[] {
  detectCycle(tasks);
...
export function fileConflicts(tasks: readonly Task[]): FileConflict[] {
  detectCycle(tasks);
  const tasksById = new Map(tasks.map((task) => [task.id, task]));
  const reaches = (from: Task, targetId: string, seen = new Set<string>()): boolean => {
  ...
  for (let leftIndex = 0; leftIndex < tasks.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < tasks.length; rightIndex += 1) {
      ...
      const files = left.files.filter((leftFile) =>
        right.files.some((rightFile) => pathsOverlap(leftFile, rightFile)),
      );
      if (files.length > 0) {
