...
async function doCommand(args: string[]): Promise<number> {
  const { flags, positionals } = parseFlags(args, new Set(['--force']));
...
async function mergeCommand(args: string[]): Promise<number> {
  const { positionals } = parseFlags(args, new Set());
    ...
    for (const task of tasks) {
      ...
      if (flags.has('--delete-branches') && !deletedBranches.has(task.branch)) {
        try {
          ...
          console.log(
            `Keeping branch ${task.branch}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      cleaned += 1;
...
function subscribeTerminal(orchestrator: SquadOrchestrator): void {
  orchestrator.on('plan:start', (event: Extract<import('@squad/core').SquadEvent, { type: 'plan:start' }>) => {
    console.log(`Planning: ${event.goal}`);
  });
  orchestrator.on('plan:log', (event: Extract<import('@squad/core').SquadEvent, { type: 'plan:log' }>) => {
    process.stdout.write(event.chunk);
  });
  orchestrator.on('plan:done', (event: Extract<import('@squad/core').SquadEvent, { type: 'plan:done' }>) => {
  ...
  orchestrator.on('run:start', (event: Extract<import('@squad/core').SquadEvent, { type: 'run:start' }>) => {
    console.log(`Running ${event.runId}.`);
  });
  orchestrator.on('task:start', (event: Extract<import('@squad/core').SquadEvent, { type: 'task:start' }>) => {
    console.log(`Task ${event.taskId} started.`);
  });
  orchestrator.on('task:log', (event: Extract<import('@squad/core').SquadEvent, { type: 'task:log' }>) => {
    process.stdout.write(`[${event.taskId}] ${event.chunk}`);
  });
  orchestrator.on('task:done', (event: Extract<import('@squad/core').SquadEvent, { type: 'task:done' }>) => {
    console.log(`Task ${event.result.id}: ${event.result.status}.`);
  });
  orchestrator.on('run:done', (event: Extract<import('@squad/core').SquadEvent, { type: 'run:done' }>) => {
    console.log(`Run ${event.runId} finished with ${event.results.length} task(s).`);
  });
}

async function writePlan(config: LoadedSquadConfig, plan: Plan): Promise<void> {
  const { planFile } = resolveSquadPaths(config);
  await mkdir(dirname(planFile), { recursive: true });
  await writeFile(planFile, `${JSON.stringify(plan, null, 2)}\n`, 'utf8');
}

async function readPlan(config: LoadedSquadConfig): Promise<Plan> {
  const { planFile } = resolveSquadPaths(config);
  ...
  if (typeof document !== 'object' || document === null || typeof (document as { goal?: unknown }).goal !== 'string') {
    throw new PlanError(`Plan file ${planFile} must contain a string goal.`);
  }
  return validatePlan(document, (document as { goal: string }).goal);
}

function allowRun(conflicts: FileConflict[], force: boolean): boolean {
...
function printConflicts(conflicts: FileConflict[]): void {
  if (conflicts.length === 0) {
    return;
  }
...
function printRuns(runs: readonly { id: string; status: string; goal: string; createdAt: string }[]): void {
  if (runs.length === 0) {
...
function printTasks(tasks: readonly TaskRecord[]): void {
  if (tasks.length === 0) {
...
function parseFlags(args: string[], allowed: ReadonlySet<string>): { flags: Set<string>; positionals: string[] } {
...
function singleRequiredArgument(values: string[], usage: string): string {
...
function missingRunPlan(runId: string): never {
...
async function ensureSquadGitignore(repoPath: string): Promise<void> {
